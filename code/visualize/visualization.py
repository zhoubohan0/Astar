...
class config:
    WIDTH = 20  # 地图列数
    HEIGHT = 20  # 地图行数
    blockLength = int(750/WIDTH)  # 绘制画面时每一个节点方块的边长
    standard_x = blockLength+100
    standard_y = 45
...
class GameBoard(QMainWindow):  # 可视化类，pyqt5进行编写。
    def __init__(self):
        ...
        for i in range(config.HEIGHT):
            ...
            for j in range(config.WIDTH):
                col.append(0)
            self.Map.append(col)
            # print(col)  #这里等效为np.zeros((HEIGHT,WIDTH))
        # print(self.Map)
    ...
    def initUI(self):
        ...
        self.label_tips = QLabel(
            "<p style='color:green'>使用说明：</p>右键单击格子选定起始点/终点,左键选定格子为墙壁或删除墙壁。\n<p style='color:green'>颜色说明：</p>\n黄色代表起点，绿色代表终点，黑色代表墙壁，灰色代表可行区域，红色代表最短路径上的节点",
            self)
        self.label_display = QLabel("", self)
        self.button_start = QPushButton("开始搜索", self)
        self.button_clearSE = QPushButton("重选始末", self)
        self.button_clearWall = QPushButton("清空墙壁", self)
        self.button_clearall = QPushButton("清空地图", self)
        self.button_saveMap = QPushButton("保存地图", self)
        self.button_loadMap = QPushButton("加载地图", self)

        # 设置控件属性
        self.label_tips.setWordWrap(True)   #label控件的文本是否能够自适应文字的大小，自动换行
        self.label_display.setWordWrap(True)    #好像只能中文能够自动分行，英文不行。听说可以用QFontMetrics获取整个字符串长度，然后按照合适长度截断
        # 设置控件样式
        ...
        # 设置控件的尺寸和位置
        self.label_tips.resize(200, 220)
        self.button_clearSE.resize(80,30)
        self.button_clearWall.resize(80,30)
        self.button_saveMap.resize(80, 30)
        self.button_loadMap.resize(80, 30)
        self.label_display.resize(200, 300)

        self.label_tips.move(config.standard_x + (config.WIDTH - 1) * config.blockLength, config.standard_y)
        self.button_start.move(config.standard_x+45 + (config.WIDTH - 1) * config.blockLength, config.standard_y+240)
        self.button_clearSE.move(config.standard_x + (config.WIDTH - 1) * config.blockLength, config.standard_y+290)
        self.button_clearWall.move(config.standard_x+100 + (config.WIDTH - 1) * config.blockLength, config.standard_y+290)
        self.button_clearall.move(config.standard_x+45 + (config.WIDTH - 1) * config.blockLength, config.standard_y+340)
        self.button_saveMap.move(config.standard_x + (config.WIDTH - 1) * config.blockLength, config.standard_y+390)
        self.button_loadMap.move(config.standard_x+100 + (config.WIDTH - 1) * config.blockLength, config.standard_y+390)
        self.label_display.move(config.standard_x + (config.WIDTH - 1) * config.blockLength, config.standard_y+440)
        # 给控件绑定事件
        ...
        self.button_clearall.clicked.connect(self.button_Clear)
        self.button_saveMap.clicked.connect(self.button_SaveMap)
        self.button_loadMap.clicked.connect(self.button_LoadMap)
        # UI初始化完成w
        self.setGeometry(0, 0, 150 + (config.WIDTH * config.blockLength - config.blockLength) + 700,
                         150 + (config.HEIGHT * config.blockLength - config.blockLength)+40)
        self.setMinimumSize(150 + (config.WIDTH * config.blockLength - config.blockLength) + 250,
                            150 + (config.HEIGHT * config.blockLength - config.blockLength)+int(600/config.WIDTH))
        self.setMaximumSize(150 + (config.WIDTH * config.blockLength - config.blockLength) + 350,
                            150 + (config.HEIGHT * config.blockLength - config.blockLength)+40)
        self.setWindowTitle('A*搜索')
        self.setWindowIcon(QIcon('star.ico'))  # 调用QIcon构造函数时，我们需要提供要显示的图标的路径(相对或绝对路径)。同时注意：使用QIcon类型必须导入此模块from PyQt5.QtGui import QIcon
        self.show()
    ...
    def mousePressEvent(self, event):
        ...
        if x >= 0 and x < config.WIDTH and y >= 0 and y < config.HEIGHT:
            if event.button() == Qt.LeftButton:
                if (x, y) != self.startPoint and (x, y) != self.endPoint:   #不是始末就是障碍
                    self.Map[y][x] = (1 if self.Map[y][x] == 0 else 0)
            if event.button() == Qt.RightButton:
                if self.Map[y][x] == 0: #不允许再次点击，只能对状态0进行右键
                    if self.startPoint == None:
                        self.startPoint = (y, x)
                        self.Map[y][x] = 2   #起点定为2
                        self.addDisplayText('添加了一个起点:(%d,%d)' % (y,x))
                    elif self.endPoint == None and self.startPoint != (y,x):   #首先点击了起始点而且与起始点不能重复再判断终点有无
                        self.endPoint = (y, x)
                        self.Map[y][x] = -1   #终点定为-1
                        self.addDisplayText('添加了一个终点:(%d,%d)' % (y,x))
            self.repaint()
    ...
    def button_StartEvent(self):
        ...
        if self.startPoint != None and self.endPoint != None:
            ...
            self.button_start.setEnabled(False)     #禁用开始
            self.button_clearSE.setEnabled(False)   #禁用清除始末
            self.button_clearWall.setEnabled(False) #禁用清除墙壁
            self.button_clearall.setEnabled(False) #禁用清除墙壁
            self.id=self.centerTimer.start(200, self)
            #算法部分
            self.search=Solver(config.WIDTH, config.HEIGHT, self.startPoint, self.endPoint, self.get_obstacles(),mode=4)
            print()
            self.search.A_star()    #得到self.search.path
            self.resultlength=len(self.search.path)
            # yi_copy=self.yi   #生成器只能list()/next()等展开一次，直接复制生成器没用
            # self.is_no_solution=list(self.yi)   #这里执行了self.yi就变成[]了！！！
            self.search.outputresult()  #这个可以在控制台输出
    ...
    def button_SaveMap(self):   #存入map文件
        for i in range(len(self.Map)):
            print(self.Map[i])
        print('*' * 100)
    ...
    def button_LoadMap(self):   #读取map文件，然后画图
        self.count = 0
        self.resultPoint = None
        try:
            with open('map.txt', 'r') as f:
                self.Map = json.loads(f.read()) #地图加载好了

            for i in range(len(self.Map)):  #打印一下self.Map
                print(self.Map[i])
            print('*' * 100)
    ...
    def button_Clear(self):
        ...
        if sender == self.button_clearSE:
            # print('清除起始点！')
            if self.startPoint:
                self.Map[self.startPoint[0]][self.startPoint[1]]=0
                self.startPoint = None
            if self.endPoint:
                self.Map[self.endPoint[0]][self.endPoint[1]]=0
                self.endPoint = None
            self.repaint()
            self.addDisplayText('已清空起始点！')
        elif sender == self.button_clearWall:
            print('清除墙壁！')

            for i in range(len(self.Map)):
                for j in range(len(self.Map[i])):
                    if self.startPoint and i==self.startPoint[0] and j==self.startPoint[1]:
                        self.Map[i][j] = 2
                    elif self.endPoint and i==self.endPoint[0] and j==self.endPoint[1]:
                        self.Map[i][j] = -1
                    else:
                        self.Map[i][j] = 0
            self.repaint()
            self.addDisplayText('已清空所有墙壁！')
        else:
            for i in range(len(self.Map)):
                for j in range(len(self.Map[i])):
                        self.Map[i][j] = 0
    ...
    def drawMap(self, qp):  # 画面绘制方法，每次地图有所改动都将重绘
        ...
        #                 qp.setBrush(QColor(255, 255, 0))    #起始点为黄色
        #             elif i == self.search.end.x and j == self.search.end.y:
        #                 qp.setBrush(QColor(100, 200, 50))   #终点为绿色
        #
        #             else:
        #                 if self.Map[i][j] == 0:
        #                     tagx = True
        ...
        #                             if k.x == i and k.y == j:
        #                                 tagx = False
        #                                 qp.setBrush(QColor(0, 100, 255))    #蓝色
        #                     if tagx:
        #                         if self.special != None:
        ...
        #没有搜索结果
            ...
            # print(self.endPoint)    #刚开始没有，鼠标点击之后有，清空棋盘后又没有
        for i in range(len(self.Map)):
            for j in range(len(self.Map[i])):   #遍历每个格子
                #直接加载的时候，startPoint和endpoint都是none

                # if self.Map[i][j] == 2:
                #     qp.setBrush(QColor(255, 255, 0))  # 到了起始点就填黄色
                # elif self.Map[i][j] == -1:
                #     qp.setBrush(QColor(100, 200, 50))  # 到了终点就填绿色

                if (i,j) == self.resultPoint:
                    qp.setBrush(QColor(255, 0, 0))    #到了路径就填红色
                elif (i,j) == self.startPoint or self.Map[i][j] == 2:
                    qp.setBrush(QColor(255, 255, 0))    #到了起始点就填黄色
                elif (i,j) == self.endPoint or self.Map[i][j] == -1:
                    qp.setBrush(QColor(100, 200, 50))   #到了终点就填绿色
                else:

                    if self.Map[i][j] == 0:
                        qp.setBrush(QColor(220, 220, 220))  #默认为0就填原来格子的灰色
                    elif self.Map[i][j] == 1:
                        qp.setBrush(QColor(0, 0, 0))    #障碍就填黑色
                    # else:
                    #     qp.setBrush(QColor(255, 0, 0))  #其余填红色

                qp.drawRect(50 + j * config.blockLength, 50 + i * config.blockLength, config.blockLength,
                            config.blockLength)

    def timerEvent(self, e):    #定时器函数
        try:
            self.count+=1
            if self.count==self.resultlength-1:
                self.killTimer(self.id)
            self.resultPoint =self.search.path[self.count]
        except Exception as e:
            self.addDisplayText('搜索结束:')
            print('搜索结束！')
            # print(e)
            if self.search.mindistance == 1e6:
                self.addDisplayText('未找到可行路径')
                print('搜索结束！')
            else:
                # self.addDisplayText('总计搜索节点数：%d' % self.search.count)
                # print(self.search.count)
                self.addDisplayText('最终路径长度：%d' % self.search.mindistance)
                best_way=str(self.search.path).replace('),',')->')
                self.addDisplayText('最终路径：%s' %best_way )
